...
class RecyclersTestCase(TestCase):
    def setUp(self):
        ...
        self.material = MaterialType.objects.create(name="Plastic", unit="kg", price_per_unit=10.0)
    ...
    def test_create_purchase(self):
        ...
        self.assertEqual(purchase.total_price, 500.0)
    ...
    def test_recycler_only_sees_own_purchases(self):
        ...
        RecyclerPurchase.objects.create(recycler=self.recycler, material_type=self.material, quantity=10, total_price=100)
        other_recycler = User.objects.create(email="other@test.com", name="Other", role="RECYCLER")
        RecyclerPurchase.objects.create(recycler=other_recycler, material_type=self.material, quantity=20, total_price=200)
        
        response = self.client.get(self.purchase_url)
        self.assertEqual(len(response.data), 1)
        self.assertEqual(float(response.data[0]['quantity']), 10.0)
